...
- **Режимы уникализации** мягкая / средняя / сильная: случайные малозаметные zoom, скорость, поворот, обрезка, цвет, шум, тон звука — свои для каждого файла; подмена метаданных без отпечатка ffmpeg; оценка «отличие от оригинала»
...
- Очистка и подмена метаданных, удаление звука
...
Площадки ищут повторы по «отпечаткам» кадров и звука, а не по байтам файла, поэтому одной очистки метаданных мало. VidUniq для **каждого файла** бросает кубики по выбранному режиму и применяет набор малозаметных изменений:

| Режим | Что меняется |
|---|---|
| **Мягкая** | zoom 1–4%, скорость ±2%, поворот до 0.4°, обрезка начала 0.1–0.4 с, цвет/гамма ±2–3%, лёгкая резкость, тон звука ±1%, громкость ±1 дБ |
| **Средняя** (по умолчанию) | zoom 3–8%, скорость ±4%, поворот до 1°, обрезка до 0.8 с, цвет ±4–6%, лёгкий шум, виньетка, другой fps, тон ±2%, ±2 дБ |
| **Сильная** | zoom 5–12%, скорость ±7%, поворот до 2°, обрезка до 1.5 с, цвет ±8–10%, шум, виньетка, резкость, fps, тон ±3%, ±3 дБ |
| Выкл (ручная) | только заданные вручную zoom, скорость и фильтры |

Во всех режимах дополнительно: случайные CRF и длина GOP (меняется структура файла), а при включённой опции метаданных — исходные теги удаляются, **отпечаток ffmpeg (`Lavf…`) убирается**, и записываются случайная дата создания и правдоподобные теги контейнера и потоков. Опционально — зеркалирование (50/50) и изменение звука.

После обработки показывается **«отличие NN%»** — расстояние между перцептивными хэшами кадров оригинала и результата (0% — визуально те же кадры). Это ориентир, а не гарантия: алгоритмы площадок закрыты и меняются. Практика показывает, что помогают также новая обложка, другое описание и пауза между публикациями одного ролика на разных аккаунтах.
...
| Звёзд | Что появится |
|---|---|
| ✅ сейчас | macOS Apple Silicon `.dmg`, режимы уникализации, все пресеты, VideoToolbox, прогресс и отмена; Telegram-бот (бета) |
...
viduniq/core/uniq.py       режимы уникализации: профили, бросок кубиков, фильтры, метаданные
viduniq/core/similarity.py оценка отличия (dHash по кадрам)
